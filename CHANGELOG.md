...
Every released version, newest first. Where a release has notes they are the authority and this
page links them rather than restating them; the older entries predate the notes and are one-line
summaries, with the tag and the built artifacts as the record behind them.

Versions follow [semantic versioning](https://semver.org/) from `1.0.0` onward. The `0.x` line
predates that promise and broke compatibility freely.
...
— twenty-two upgrade items, ordered by how likely each is to reach you. The notes' own top four,
and one further down that bites early:
...
- `Config`, `ContinueResult`, `FlushResult`, `Health` and `SinkLosses` are keyword-only, so
  positional construction raises `TypeError` — and `case Health(a, b):` no longer matches, since
  keyword-only empties `__match_args__`.
- A bad sink argument is refused where it is written rather than where it first breaks delivery,
  so an upgrade can fail at process start where `0.10.x` started fine. It reaches every
  HTTP-family sink and every subclass built on one.
...
- Further down but early to bite: `log_foundry.sinks.util` is deleted with no alias, so
  `MemorySink`, `NullSink` and `StderrSink` move to `log_foundry.sinks.memory`, `…null` and
  `…stdout` — an `ImportError` at import time.

The quietest one is worth reading for even if nothing above applies: `continue_trace()` is now
one-shot, applying to the next root span and not surviving it. No exception and no counter, just
shorter traces than you expected.
...
queue and stream sinks, and the databases. Published as **`log-foundry`** — the distribution
carried that name from the very first upload, because PyPI rejected `log-forge` before anything
was published — while the *import* package was still `log_forge` until `0.2.0` below.

**Yanked on PyPI**, reason `Outdated API`: a range like `>=0.1.0` skips it silently and an exact
pin installs with a warning. It is the only yanked release.
