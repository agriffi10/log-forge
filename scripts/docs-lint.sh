...
# answer is another cut: the last one was structural and took the file to a third of
# its size without losing a fact.
...
# for under ~40 lines. A RATCHET AT THE MEASURED LEVEL rather than the template's 150:
# three docs already sit above that (SPEC-028 at 267, SPEC-030 at 230, SPEC-032 at 165),
# written before anything checked, and failing main on three historical docs on day one
# is how a new gate gets switched off. Trimming any of them LOWERS this number in the
  ...
  awk -v claude="$CLAUDE" -v reg="$REGISTER" '
    ...
    NR == FNR {
      ...
      if (in_sec && $0 ~ /^- \*\*/) {
        s = $0
        sub(/^- \*\*/, "", s)
        # The closing ** is the first one NOT followed by another *. A label ending in an
        # italic ("...skip *work*") is stored as *work***, where a plain index(s, "**")
        # finds the pair straddling the italic own asterisk and truncates the label by
        # one character. That reads as a digest line whose entry is missing AND an entry
        # whose digest line is missing — two failures naming almost the same string, for
        # a label that is in fact correct.
        i = 0; p = 1
