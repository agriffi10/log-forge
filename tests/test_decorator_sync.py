...
pytestmark = pytest.mark.skipif(
    not (hasattr(log_forge, "trace") and hasattr(log_forge, "configure")),
    reason="log_forge.trace / configure not implemented yet",
)


def test_single_span_records_start_end_ok(fake_sink) -> None:
    log_forge.configure(service="t", sink=fake_sink)

    @log_forge.trace(name="work")
    def work() -> int:
        return 42
...
def test_nested_calls_share_trace_and_link_parent(fake_sink) -> None:
    ...
    @log_forge.trace(name="child")
    def child() -> None:
        return None

    @log_forge.trace(name="parent")
    def parent() -> None:
        child()
