...
The distribution is published as ``log-foundry``; the import name remains ``log_foundry``.
"""
...
def test_version_is_a_string() -> None:
    assert isinstance(log_foundry.__version__, str)
    assert log_foundry.__version__


def test_version_is_a_public_pep440_version() -> None:
    # No local segment (+<hash>) — PyPI and TestPyPI reject uploads that carry one.
    assert _PEP440_PUBLIC.match(log_foundry.__version__), log_foundry.__version__
    assert "+" not in log_foundry.__version__
...
def test_version_matches_distribution_metadata() -> None:
    try:
        ...
        pytest.skip("log-foundry is not installed in this environment")
    assert log_foundry.__version__ == expected


def test_version_is_exported() -> None:
    assert "__version__" in log_foundry.__all__
...
def test_version_falls_back_when_not_installed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        reloaded = importlib.reload(log_foundry)
        assert reloaded.__version__ == "0.0.0"
    finally:
        monkeypatch.undo()
        importlib.reload(log_foundry)  # restore the real version for later tests
