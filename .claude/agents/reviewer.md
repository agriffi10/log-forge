...
dependencies, the invariants it cites and the decisions it claims to follow; for a plan, the spec and
`docs/component-inventory.md`; for a diff, the spec's acceptance criteria, the numbered promises in
`docs/invariants.md`, and `docs/best-practices/python/python.md`. Review the artifact against those
sources only — you are given no authoring rationale, on purpose.
...
- Run this repo's gates against the branch: `poetry run ruff check .`, `poetry run mypy`,
