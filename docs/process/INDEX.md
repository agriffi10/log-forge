...
This table is the **authority** for the always-loaded set. `scripts/docs-lint.sh` derives that set
from it — the set it holds to a byte budget — and requires `CLAUDE.md`'s imports to match it row for
row, so adding a row is a visible act that costs every session: make the case in the PR. Nothing in
this table other than `CLAUDE.md` may carry a bare `@path` import, because an import here pulls its
target in at launch too, through recursion, which is the cost this layout exists to avoid.
...
| `docs/process/authoring-a-spec.md` | what makes a spec buildable: scope, FRs with binary criteria that cite an invariant, size, no Open Questions | when writing or revising a spec |
...
every row is a file — an unrouted part is where the next paragraph of process accretes unread, so
`scripts/docs-lint.sh` holds the sets equal.
...
|---|---|---|---|
| Always | `CLAUDE.md` + its two imports (the table above) | every session | conventions, the key-decisions **areas**, the session rhythm, what loads when |
| Decisions | `docs/decisions/<area>.md` | the *fences* of your area, then the entry you need | the settled decisions **in full** — reasoning, rejected alternatives, every "do NOT build" fence |
...
- **Never** read `architecture.md`, a `docs/decisions/` area file or a delivery doc whole — pull the
  one section, fence or entry you need. Each is built so that is possible.
...
to pull a part before working in its area lives here and in `CLAUDE.md`; the rule only catches the
session that forgot.
