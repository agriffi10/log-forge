...
- **Size: aim for 3–6 FRs, and split above 8.** A spec is one coherent slice of behavior, not a
  ...
  tree, not a standing claim about today's corpus — §5 forbids a rule resting on a number that
...
- **A register is grouped by AREA; ordering it by spec number turns it into a changelog.** The
  ...
  reads as disposable gets treated as disposable. **Paid 2026-09-02:** `docs/decisions.md` holds the
  entries, grouped by area, and `CLAUDE.md`'s Key Decisions is a one-line digest over it, grouped by
  the same areas. A completion **replaces or extends the clause for its area** rather than appending
  another entry — appending is what took `CLAUDE.md` from 7,350 bytes at `ad898fc8` to 89,340 at
  `e60b60d`, with this rule stated here throughout. Both ends are anchored so a reader can re-measure
  them; neither the entry count nor the area count is stated here, because both move every time a
  spec closes.
- **`scripts/docs-lint.sh` enforces the structural half of these rules, and is a LOCAL PRE-PUSH
  ...
  **unit** — a bullet with its continuations — to a length, and refuses any construct in that section but an area heading, a bullet, a continuation, a blank line and plain intro prose; requires `docs/decisions.md` to carry a `###` entry for every digest line and a
  ...
  to have a delivery doc; checks that the pointers out of `CLAUDE.md` resolve; and refuses one
  ...
  sub-shape, not the rule: an undated volatile number is invisible to it and stays this section's
  ...
  **A threshold can be invalidated by its own success, not only by being wrong.** A cap
  ...
  fences to buy room for its own, which is the gate causing the damage it exists to prevent. How much
