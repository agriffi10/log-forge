...
`kw_only` empties `__match_args__`, so positional pattern matching on these five raises `TypeError` from the `match` statement, against a subject of that type, rather than falling through; a subject of any other type still falls through as before.
