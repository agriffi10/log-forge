...
- **No record means refused.** The default is the whole point of the spec: every gap must fail
  ...
  **Amended in delivery, escalated and approved:** every *lifecycle* path stamps, so this flat
  ...
  safe is that a forked child marks everything it inherited and the parent never recorded
  **before any handler runs**, so "unrecorded" is unclaimable there rather than merely
  unreleasable. See the delivery doc.
...
def _mark_inherited() -> None: ...            # a fork handler; setdefaults _FOREIGN over what
                                              # the parent never recorded, so "no record" is
                                              # unclaimable in a child
