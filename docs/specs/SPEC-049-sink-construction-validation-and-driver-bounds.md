...
containing CR/LF is written straight into `Authorization` at `http.py:844`, which is the same
...
got it wrong: the action line is built with `json.dumps` (`elasticsearch.py:88`), so the NDJSON
