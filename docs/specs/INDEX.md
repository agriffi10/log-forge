...
| [SPEC-031](SPEC-031-audit-small-corrections.md) | Audit Small Corrections | Draft | SPEC-004, SPEC-008, SPEC-009, SPEC-020, SPEC-030 |
...
- **Post-close sink behaviour:** SPEC-032 — the arc's coda, and the only spec here whose subject
  ...
  so `shutdown()` is a no-op, `atexit` is never registered, the sink is never closed and every event
  is lost — with `health()` reading all-clear, because each field describes a worker that does not
  exist. Found in SPEC-032's review and out of its scope (it is `decorator.py` lifecycle, not sink
