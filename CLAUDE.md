...
- `@docs/process/completion-ritual.md` — the seven steps at spec completion + the doc-hygiene rules
...
**PRs & main:** building a spec **authorizes the whole landing** — the push of the reviewed branch, the PR, the watch and the merge on green are the build's last steps, not decisions to confirm, so do not stop to ask for them and do not stop at announcing them. Before pushing, get the diff through the review gate above, and get this repo's six gates green locally — `poetry run ruff check .`, `poetry run mypy`, `poetry run pytest`, `sh scripts/spec-lint.sh`, and — **before every PR, since nothing in CI runs either** — **`sh scripts/docs-lint.sh`** and **`poetry run python scripts/docstring-lint.py`** (plus each one's `-test.sh` corpus whenever you touch that linter, and `sh scripts/pr-queue-test.sh` if you touched the queue or its hook, `sh scripts/pr-queue/install-test.sh` if you touched its installer). **That list is a floor, not the set** — also run locally whatever CI will run on the branch. **`ruff format` is NOT among them**: it is not a CI gate and this repo is not clean under it, so format only the files you edited (`@docs/process/operational-traps.md`). **When several agent sessions share this repo** — they routinely do — the PR queue serialises the remote: one PR open at a time, in the order asked, `main` green before the next, and you join the line only once your gates and reviews are green, because the queue is not a review. Install and read `scripts/pr-queue/` first; uninstalled it enforces nothing, silently. Once installed its `pre-push` hook adds a second refusal to the lock's — a push whose commits do not contain the remote's current `main` — so fetch and rebase **inside** the lock, not before joining the line. Protocol: `scripts/pr-queue/PROTOCOL.md`; brief each session from `@docs/templates/multi-agent-briefing.md`; the rest: `@docs/process/several-agents.md`. Watch every PR to completion and merge it as soon as CI is green — never open-and-abandon. **Key the watch on the current head sha** — a bare `gh pr checks --watch` can exit clean against the *previous* commit's checks. `main` is always watched: after any merge confirm it went green, and if `main` fails, diagnose immediately and fix it with a new PR before anything else.

**On spec completion:** run the seven steps in `@docs/process/completion-ritual.md` — status, INDEX row, delivery doc, inventory row, the register entry with its Contents row **before** its fence in the area file, and — if the spec changed anything a caller can see — the pending release notes, the one release artifact no gate holds to the code, and a prune of `## Specs` above (never an append). A reversal updates the old entry in place and leaves a superseded marker at every other site still stating the old claim.
