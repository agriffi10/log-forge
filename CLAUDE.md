...
**Current work:** SPEC-054 — assembly, decoration and echo residue.
...
- **Unit of work = a decorated call** — `@log_foundry.trace`; the outermost call starts a trace, every call is a span within it. The name is resolved once at decoration; a misordered descriptor or a non-callable is refused there. (arch §4, SPEC-054)
...
- **An event is safe by construction — coerced and bounded once at assembly, not per sink** — `build_event` runs every value through `sanitize.py`, so the bare `json.dumps` calls in `sinks/` are correct by consequence. The unserializable fallback is a type name, never `repr()`. Ceilings bound per *value*. No string leaves assembly unencodable (a surrogate becomes U+FFFD, marked), and a hostile key costs only itself. (SPEC-017, SPEC-054)
