...
- **A rule with no gate is a rule that rots, and the cap that rots is the one contradicted by its neighbour** — the ≤3-sentence docstring cap sat beside "reasoning ... belongs *in* the docstring" and the code followed the neighbour, a third of defs over it. Re-scoped to the **summary line**, where compliance was already total, so `scripts/docstring-lint.py` shipped green. A corpus mutation-tests every **exemption**, not just every check — and **a gate's scoping test binds to what it scopes**: tested sentence-wide it is defeated by the word an author reaches for while fixing the very defect. (SPEC-052, SPEC-053)
